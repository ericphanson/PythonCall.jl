module PythonCall_Exec

using ..PythonCall_Base
using ..PythonCall_Base: pynew, pycopy!, pydel!
...
_is_expr(ex; head=nothing, nargs=nothing) = ex isa Expr && (head === nothing || ex.head == head) && (nargs === nothing || length(ex.args) == nargs)
_parse_pair_expr(ex) = _is_expr(ex; head=:call, nargs=3) && ex.args[1] === :(=>) ? (ex.args[2], ex.args[3]) : nothing

function _pyeval_macro_args(arg, filename, mode)
    # separate out inputs => code => outputs (with only code being required)
    if (arg2 = _parse_pair_expr(arg)) !== nothing
        lhs, rhs = arg2
        if (arg3 = _parse_pair_expr(rhs)) !== nothing
            inputs = lhs
            code, outputs = arg3
    ...
    if inputs === nothing
        ...
        for input in inputs
            if input isa Symbol
                push!(locals, input => input)
            elseif _is_expr(input, head=:(=), nargs=2) && input.args[1] isa Symbol
                push!(locals, input.args[1] => input.args[2])
            else
...
macro pyexec(arg)
    ...
    if outputs === nothing
        ...
        for output in outputs
            if _is_expr(output, head=:(=), nargs=2)
                lhs, rhs = output.args
