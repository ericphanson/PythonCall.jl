...
_showerror_julia(args...; kw...) = false

function _showerror(io::IO, e::PyException, bt; backtrace::Bool=true)
    print(io, "Python: ")
    ...
    if true # TODO: CONFIG.auto_sys_last_traceback
        try
            sys = pyimport("sys")
            sys.last_type = e.t
            sys.last_value = e.v
    ...
    if !_showerror_julia(io, e; backtrace)
        # print the type name
        try
